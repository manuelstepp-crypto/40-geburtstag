...
function saveGuests() {
localStorage.setItem(STORAGE_KEY, JSON.stringify(allGuests));
debouncedGhSave('Übernachtung aktualisiert');
}

function updateStats() {
const yes = allGuests.filter(g => g.response === 'yes');
const el = id => document.getElementById(id);
if (el('statTotal')) el('statTotal').textContent = yes.length;
}
...
const ueb = (card.querySelector('.guest-uebernachtung')?.textContent || '').toLowerCase();
card.style.display = (name.includes(q) || ueb.includes(q)) ? '' : 'none';
});
}
...
const uebHtml = ueb
? `<div class="guest-uebernachtung">🏠 ${escapeHtml(ueb)}</div>`
: `<div class="guest-uebernachtung guest-uebernachtung-empty">🏠 Übernachtung noch offen</div>`;
...
setTimeout(() => document.getElementById('guestUebernachtung').focus(), 50);
}
